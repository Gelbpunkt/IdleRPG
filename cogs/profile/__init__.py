...
**Guild**: `{guild}`"""
    ...
    @locale_doc
    async def inventory(
            self, ctx, itemtype: Optional[str.title] = "All", lowest: IntFromTo(0, 100) = 0,
        ...
        if not itemtype in self.bot.config.item_types + ["All"]:
            return await ctx.send(_("Please select a valid item type or `all`. Available types: `{all_types}`").format(
                all_types=", ".join(self.bot.config.item_types)))
        if itemtype == "All":
