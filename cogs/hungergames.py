...
class HungerGames():
    ...
    async def hungergames(self, ctx, player_count: int=10):
        ...
        players = [ctx.author]
        msg = await ctx.send(f"{ctx.author.mention} started a game of Hunger Games! React with :shallow_pan_of_food: to join the game! **1/{player_count}**")
        await msg.add_reaction("\U0001f958")

        def check(reaction, user):
            return user not in players and reaction.message.id == msg.id and not user.bot
        ...
        while len(players) < player_count:
            ...
            players.append(user)
            await msg.edit(content=f"{ctx.author.mention} started a game of Hunger Games! React with :shallow_pan_of_food: to join the game! **{len(players)}/{player_count}**")
