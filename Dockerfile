...
RUN apk add --no-cache curl git util-linux jpeg-dev zlib-dev cairo-dev && \
    ...
    curl -L "https://github.com/Gelbpunkt/alpine-stockfish/raw/master/stockfish" -o stockfish && \
    chmod +x stockfish && \
    apk del curl
